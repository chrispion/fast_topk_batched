...
#    endif
#    include <windows.h>
// Remove any possible macro interference
#    undef min
#    undef max

typedef void (*fast_topk_func)(float *, int, int, int, int *);
static fast_topk_func fast_topk_ptr   = nullptr;
static bool           fast_topk_tried = false;

void load_fast_topk_dll() {
    if (fast_topk_tried) {
        return;
    }

    // Check local directory first
    HMODULE hModule = LoadLibraryA("fast_topk_batched.dll");

    if (!hModule) {
        // Fallback: check EXE directory
        char path[MAX_PATH];
        GetModuleFileNameA(NULL, path, MAX_PATH);
        std::string dir      = std::string(path);
        dir                  = dir.substr(0, dir.find_last_of("\\/"));
        std::string dll_path = dir + "\\fast_topk_batched.dll";
        hModule              = LoadLibraryA(dll_path.c_str());
    }

    if (hModule) {
        fast_topk_ptr = (fast_topk_func) GetProcAddress(hModule, "fast_topk_batched");
        if (fast_topk_ptr) {
            printf("\n>>> [SUCCESS] FastTopK Kernel Linked at %p <<<\n", (void *) fast_topk_ptr);
        } else {
            printf("\n>>> [ERROR] Found DLL but function 'fast_topk_batched' is missing <<<\n");
        }
    } else {
        printf("\n>>> [DEBUG] FastTopK DLL not found. Using default CPU sort. <<<\n");
    }
    fast_topk_tried = true;
}
#endif
// ------------------------
...
static void llama_token_data_array_partial_sort_inplace(llama_token_data_array * cur_p, int npartial) {
    if (npartial <= 0 || cur_p->size == 0) {
        return;
    }

#ifdef _WIN32
    load_fast_topk_dll();
    if (fast_topk_ptr && npartial > 1 && (size_t) npartial <= cur_p->size) {
        const int V = (int) cur_p->size;

        // 1. Create a workspace for logits and a backup of the original data
        std::vector<float>            logits_buf(V);
        std::vector<llama_token_data> original_data(cur_p->data, cur_p->data + V);

        for (int i = 0; i < V; ++i) {
            logits_buf[i] = cur_p->data[i].logit;
        }

        // 2. Output buffer for the kernel
        std::vector<int> indices_buf(npartial);

        // 3. EXECUTE KERNEL
        // If your kernel expects 'float*', this is correct.
        // If it crashes instantly here, your kernel might expect 'double*'.
        try {
            fast_topk_ptr(logits_buf.data(), 1, V, npartial, indices_buf.data());
        } catch (...) {
            fprintf(stderr, "[FastTopK] CRITICAL: Kernel execution failed!\n");
            return;
        }

        // 4. SAFE RECONSTRUCTION
        // Instead of swapping in-place (which can corrupt if indices overlap),
        // we rebuild the front of the array from our backup.
        std::sort(indices_buf.begin(), indices_buf.end(),
                  [&](int a, int b) { return original_data[a].logit > original_data[b].logit; });
        for (int i = 0; i < npartial; ++i) {
            int found_idx = indices_buf[i];
            if (found_idx >= 0 && found_idx < V) {
                cur_p->data[i] = original_data[found_idx];
            }
        }

        cur_p->size   = (size_t) npartial;
        cur_p->sorted = true;
        return;
    }
#endif

    // --- Original Fallback (Do not change) ---
    static const auto comp = [](const llama_token_data & a, const llama_token_data & b) {
        return a.logit > b.logit;
    };
    if (npartial <= 128) {
...
static bool llama_sampler_empty_backend_init(struct llama_sampler * smpl, ggml_backend_buffer_type_t buft) {
    GGML_UNUSED(smpl);
    GGML_UNUSED(buft);
...
static void llama_sampler_empty_backend_accept(struct llama_sampler * smpl,
                                               ggml_context *         ctx,
                                               ggml_cgraph *          gf,
...
static void llama_sampler_empty_backend_apply(struct llama_sampler *      smpl,
...
static bool llama_sampler_backend_support(llama_sampler * smpl, ggml_backend_buffer_type_t buft) {
    ...
    for (int i = 0; i < ggml_graph_n_nodes(gf); i++) {
        ...
        if (!ggml_backend_dev_supports_op(device, op)) {
            LLAMA_LOG_WARN("%s: device '%s' does not have support for op %s needed for sampler '%s'\n", __func__,
                           ggml_backend_dev_name(device), ggml_op_name(op->op), smpl->iface->name(smpl));
...
static bool llama_sampler_chain_backend_init(struct llama_sampler * smpl, ggml_backend_buffer_type_t buft) {
    auto * chain = (llama_sampler_chain *) smpl->ctx;
...
static void llama_sampler_chain_backend_accept(struct llama_sampler * smpl,
                                               ggml_context *         ctx,
                                               ggml_cgraph *          gf,
                                               struct ggml_tensor *   selected_token) {
    auto * chain = (llama_sampler_chain *) smpl->ctx;
...
static void llama_sampler_chain_backend_apply(struct llama_sampler *      smpl,
                                              struct ggml_context *       ctx,
                                              struct ggml_cgraph *        gf,
                                              struct llama_sampler_data * data) {
    auto * chain = (llama_sampler_chain *) smpl->ctx;
...
static bool llama_sampler_greedy_backend_init(struct llama_sampler * smpl, ggml_backend_buffer_type_t buft) {
    auto * sctx = (llama_sampler_greedy *) smpl->ctx;
...
static void llama_sampler_greedy_backend_apply(struct llama_sampler *      smpl,
                                               struct ggml_context *       ctx,
                                               struct ggml_cgraph *        gf,
                                               struct llama_sampler_data * data) {
    GGML_UNUSED(gf);
...
static bool llama_sampler_dist_backend_init(struct llama_sampler * smpl, ggml_backend_buffer_type_t buft) {
    auto * sctx = (llama_sampler_dist *) smpl->ctx;
...
static void llama_sampler_dist_backend_apply(struct llama_sampler *      smpl,
                                             struct ggml_context *       ctx,
                                             struct ggml_cgraph *        gf,
                                             struct llama_sampler_data * data) {
    GGML_UNUSED(gf);
...
static bool llama_sampler_top_k_backend_init(struct llama_sampler * smpl, ggml_backend_buffer_type_t buft) {
    auto * sctx = (llama_sampler_top_k *) smpl->ctx;
...
static void llama_sampler_top_k_backend_apply(struct llama_sampler *      smpl,
                                              struct ggml_context *       ctx,
                                              struct ggml_cgraph *        gf,
                                              struct llama_sampler_data * data) {
    auto * sctx = (llama_sampler_top_k *) smpl->ctx;
...
static bool llama_sampler_top_p_backend_init(struct llama_sampler * smpl, ggml_backend_buffer_type_t buft) {
    auto * sctx = (llama_sampler_top_p *) smpl->ctx;
...
static void llama_sampler_top_p_backend_apply(struct llama_sampler *      smpl,
                                              struct ggml_context *       ctx,
                                              struct ggml_cgraph *        gf,
                                              struct llama_sampler_data * data) {
    auto * sctx = (llama_sampler_top_p *) smpl->ctx;
...
static bool llama_sampler_min_p_backend_init(struct llama_sampler * smpl, ggml_backend_buffer_type_t buft) {
    auto * sctx = (llama_sampler_min_p *) smpl->ctx;
...
static void llama_sampler_min_p_backend_apply(struct llama_sampler *      smpl,
                                              struct ggml_context *       ctx,
                                              struct ggml_cgraph *        gf,
                                              struct llama_sampler_data * data) {
    auto * sctx = (llama_sampler_min_p *) smpl->ctx;
...
static void llama_sampler_backend_temp_sampling(struct ggml_context *       ctx,
                                                struct ggml_cgraph *        gf,
                                                struct llama_sampler_data * data,
                                                float                       temp) {
    if (temp <= 0.0f) {
...
static bool llama_sampler_temp_backend_init(struct llama_sampler * smpl, ggml_backend_buffer_type_t buft) {
    auto * sctx = (llama_sampler_temp *) smpl->ctx;
...
static void llama_sampler_temp_backend_apply(struct llama_sampler *      smpl,
                                             struct ggml_context *       ctx,
                                             struct ggml_cgraph *        gf,
                                             struct llama_sampler_data * data) {
    auto * sctx = (llama_sampler_temp *) smpl->ctx;
...
static bool llama_sampler_temp_ext_backend_init(struct llama_sampler * smpl, ggml_backend_buffer_type_t buft) {
    auto * sctx = (llama_sampler_temp_ext *) smpl->ctx;
...
static void llama_sampler_temp_ext_backend_apply(struct llama_sampler *      smpl,
                                                 struct ggml_context *       ctx,
                                                 struct ggml_cgraph *        gf,
                                                 struct llama_sampler_data * data) {
    auto * sctx = (llama_sampler_temp_ext *) smpl->ctx;
...
struct llama_sampler * llama_sampler_init_penalties(int32_t penalty_last_n,
    ...
    const bool is_empty =
        (penalty_last_n == 0 || (penalty_repeat == 1.0f && penalty_freq == 0.0f && penalty_present == 0.0f));
...
static void llama_sampler_logit_bias_backend_apply(struct llama_sampler *      smpl,
                                                   struct ggml_context *       ctx,
                                                   struct ggml_cgraph *        gf,
                                                   struct llama_sampler_data * data) {
    GGML_UNUSED(gf);
...
static bool llama_sampler_logit_bias_backend_init(struct llama_sampler * smpl, ggml_backend_buffer_type_t buft) {
    auto * sctx = (llama_sampler_logit_bias *) smpl->ctx;
